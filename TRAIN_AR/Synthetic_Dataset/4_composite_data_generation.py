...
annotations = []
annotation_id = 0
def composite_images_centered(background, background_id, images_to_insert, grid_pixel_size, folder_id_list):
    print("a")
    ...
    for i in range(len(images_to_insert)):
        ...
        bbox = [x1, x2, grid_pixel_size, grid_pixel_size]
        annotations.append({
            "id": annotation_id,
            "bbox": bbox,
            ...
            "segmentation": [],
            "ignore": 0,
            "area": grid_pixel_size*grid_pixel_size,  # bbox의 너비와 높이로 area 계산 (수정 가능)
            "iscrowd": 0,
...
try:
    ...
    for bg_image in background_images:
        for _ in range(2):
            ...
            composite_img = composite_images_centered(bg_image.copy(), i, bg_folder_images, random_number, folder_id_list)
